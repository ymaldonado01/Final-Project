...
CYBLE_DISC_SRVC_INFO_T cyBle_serverInfo[CYBLE_SRVI_COUNT] = /*4*/
{
    ...
    {{CYBLE_GATT_INVALID_ATTR_HANDLE_VALUE, CYBLE_GATT_INVALID_ATTR_HANDLE_VALUE}, CYBLE_UUID_ALERT_NOTIFICATION_SERVICE},
    ...
    {{CYBLE_GATT_INVALID_ATTR_HANDLE_VALUE, CYBLE_GATT_INVALID_ATTR_HANDLE_VALUE}, CYBLE_UUID_CYCLING_SPEED_AND_CADENCE_SERVICE},
...
Parameters:
 CYBLE_GATTC_HANDLE_VALUE_NTF_PARAM_T * eventParam - event parameter
...
Parameters:
 CYBLE_GATTC_HANDLE_VALUE_IND_PARAM_T * eventParam - event parameter
...
Parameters:
 CYBLE_GATTC_READ_RSP_PARAM_T * eventParam - event parameter
    ...
    switch(eventCode)
    {
        ...
        ***********************************************************/
        ...
        #if(CYBLE_MODE_PROFILE)
        {
            ...
            bleSsPowerLevel.blePwrLevelInDbm = CYBLE_TX_POWER_LEVEL;
        ...
        ***********************************************************/        
        case CYBLE_EVT_GATT_CONNECT_IND:
        	...
        	(void) memcpy (cyBle_attValuesCCCD, cyBle_attValuesCCCDFlashMemory[cyBle_connHandle.bdHandle], 
...
void CyBle_ReadByGroupEventHandler(CYBLE_GATTC_READ_BY_GRP_RSP_PARAM_T *eventParam)
{
        ...
        (0u != (cyBle_eventHandlerFlag & CYBLE_AUTO_DISCOVERY)))
    {
        ...
        if((dataLength == CYBLE_DISC_SRVC_INFO_LEN) || (dataLength == CYBLE_DISC_SRVC_INFO_128_LEN))
        {
            ...
            if((discSrvcInfo != NULL) && (discSrvcInfo->range.endHandle == CYBLE_GATT_ATTR_HANDLE_END_RANGE))
            {
...
#endif /* CYBLE_TPS_CLIENT */
#ifdef CYBLE_UDS_CLIENT
    if((cyBle_disCount == (uint8) CYBLE_SRVI_UDS_DCI) || (cyBle_disCount == (uint8) CYBLE_SRVI_UDS_UCP))
    {
        uint8 charIdx;
        if(cyBle_disCount == (uint8) CYBLE_SRVI_UDS_DCI)
        {
            charIdx = CYBLE_UDS_DCI;
        }
        else
        {
            charIdx = CYBLE_UDS_UCP;
        }
            ...
            && (0u != (cyBle_eventHandlerFlag & CYBLE_AUTO_DISCOVERY)))
    {
        ...
        for(i = 0u; i < attrLength; i += dataLength)
        {
            ...
            else
            {
                ...
                #ifdef CYBLE_UDS_CLIENT
                    if((cyBle_disCount == (uint8) CYBLE_SRVI_UDS_DCI) || 
                        (cyBle_disCount == (uint8) CYBLE_SRVI_UDS_UCP))
                    {
...
CYBLE_API_RESULT_T CyBle_NextCharDscrDiscovery(void)
{
    ...
    while(((charRange.startHandle == (CYBLE_GATT_INVALID_ATTR_HANDLE_VALUE + 1u)) || (charRange.endHandle == CYBLE_GATT_INVALID_ATTR_HANDLE_VALUE) ||
