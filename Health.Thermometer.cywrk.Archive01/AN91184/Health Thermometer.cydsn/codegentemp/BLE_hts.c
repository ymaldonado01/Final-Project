...
{
    CYBLE_HTS_DESCR_INDEX_T locDescrIndex;

    if(CYBLE_UUID_CHAR_CLIENT_CONFIG == discDescrInfo->uuid.uuid16)
    {
        locDescrIndex = CYBLE_HTS_CCCD;
    }
    else if(CYBLE_UUID_CHAR_VALID_RANGE == discDescrInfo->uuid.uuid16)
    {
        locDescrIndex = CYBLE_HTS_VRD;
    }
    else    /* Not supported descriptor */
    {
        locDescrIndex = CYBLE_HTS_DESCR_COUNT;
    }

    if(locDescrIndex < CYBLE_HTS_DESCR_COUNT)
    {
        if(cyBle_htsc.charInfo[discoveryCharIndex].descrHandle[locDescrIndex] == CYBLE_GATT_INVALID_ATTR_HANDLE_VALUE)
        {
            cyBle_htsc.charInfo[discoveryCharIndex].descrHandle[locDescrIndex] = discDescrInfo->descrHandle;
        }
    ...
    if((NULL != CyBle_HtsApplCallback) && (CYBLE_GATT_INVALID_ATTR_HANDLE_VALUE != cyBle_htscReqHandle))
    {
            ...
            locCharIndex++)
        {
            ...
            else
            {
                ...
                for(locDescrIndex = CYBLE_HTS_CCCD; (locDescrIndex < CYBLE_HTS_DESCR_COUNT) &&
                   (locReqHandle == 0u); locDescrIndex++)
                {
                    if(cyBle_htscReqHandle == cyBle_htsc.charInfo[locCharIndex].descrHandle[locDescrIndex])
                    {
                        CYBLE_HTS_DESCR_VALUE_T locDescrValue;
                        
                        locDescrValue.connHandle = eventParam->connHandle;
                        locDescrValue.charIndex = locCharIndex;
                        locDescrValue.descrIndex = locDescrIndex;
